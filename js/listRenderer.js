...
function listRenderer(objAPI_a, strFormID_a, objParameters_a)
{
	...
	function createFloatingEditor()
	{
		...
		if (!m_objFloatingEditor)
		{
			...
			var strEditorHTML = '<input type="text" id="' + strEditorID + '" class="ge-floating-editor gscr-floating-editor gscr-form-control" style="position: absolute; z-index: 1000; display: none;" />';
	...
	function renderToolbar()
	{
		...
		if (m_blnAllowSearch)
		{
			...
			strHTML += '<input type="text" class="ge-list-search gscr-form-control" placeholder="Search..." value="' + m_strSearch + '">';
			strHTML += '<button class="ge-list-clear gscr-btn gscr-btn-secondary" title="Clear search">×</button>';
			strHTML += '<button class="ge-list-refresh gscr-btn gscr-btn-secondary" title="Refresh">⟳</button>';
			strHTML += '</div>';
			...
			{
				objOperation = m_arrOperations[intIndex];
				strHTML += '<button class="ge-operation ge-operation-' + objOperation.code + ' gscr-btn gscr-btn-primary" data-operation="' + objOperation.code + '"';
	...
	function renderListView()
	{
		...
		strHTML += '<table class="gscr-list-table gscr-table">';
	...
	function renderPagination()
	{
		...
		if (m_strFlow === 'PAGED')
		{
			...
			strHTML += '<button class="ge-list-first gscr-btn gscr-btn-secondary"' + (!blnHasPrevious ? ' disabled' : '') + '>⟪ First</button>';
			strHTML += '<button class="ge-list-previous gscr-btn gscr-btn-secondary"' + (!blnHasPrevious ? ' disabled' : '') + '>‹ Previous</button>';
			strHTML += '<button class="ge-list-next gscr-btn gscr-btn-secondary"' + (!blnHasNext ? ' disabled' : '') + '>Next ›</button>';
			strHTML += '</div>';
